...
async function fetchIndeedRSS(query: string): Promise<RSSItem[]> {
  try {
    ...
    const res = await fetch(url, {
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; VukaCareerHub/1.0)' },
    })
    if (!res.ok) return []
...
function buildEmail({
  ...
  date: string
  urgentBursaries: Bursary[]
  pastBursaries: Bursary[]
  urgentLearnerships: Learnership[]
  pastLearnerships: Learnership[]
  newLearnerships: RSSItem[]
  newBursaries: RSSItem[]
}) {
...
<div style="max-width:600px;margin:0 auto;padding:24px 16px">
  ...
  <!-- Summary bar -->
  <div style="background:white;border:1px solid #e2e8f0;border-top:none;padding:16px 24px;display:flex;gap:24px">
    <table width="100%"><tr>
      ${row('Closing ≤45 days', String(urgentBursaries.length + urgentLearnerships.length), urgentBursaries.length + urgentLearnerships.length > 0 ? '#d97706' : '#16a34a')}
      ${row('Expired (still active)', String(pastBursaries.length + pastLearnerships.length), pastBursaries.length + pastLearnerships.length > 0 ? '#dc2626' : '#16a34a')}
      ${row('New from job boards', String(newLearnerships.length + newBursaries.length), '#fb923c')}
    </tr></table>
  </div>
  ...
  <div style="background:white;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 16px 16px;padding:24px">
    ...
    <p style="font-size:12px;color:#64748b;margin:0 0 12px">These records are still marked active but their deadline has passed. Update the deadline, or toggle inactive.</p>
    ${pastBursaries.length > 0 ? `
    <div style="font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Bursaries</div>
    <table width="100%" cellpadding="0" cellspacing="0">
      ${pastBursaries.map((b) => pastRow(b.name, b.deadline, `${ADMIN_URL}/admin/bursaries/${b.id}/edit`)).join('')}
    </table>` : ''}
    ${pastLearnerships.length > 0 ? `
    <div style="font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1px;margin:12px 0 6px">Learnerships</div>
    <table width="100%" cellpadding="0" cellspacing="0">
      ${pastLearnerships.map((l) => pastRow(l.title, l.deadline, `${ADMIN_URL}/admin/learnerships/${l.id}/edit`)).join('')}
    </table>` : ''}
    ...
    ${urgentBursaries.length > 0 ? `
    <div style="font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Bursaries</div>
    <table width="100%" cellpadding="0" cellspacing="0">
      ${urgentBursaries.map((b) => urgentRow(b.name, b.deadline, `${ADMIN_URL}/admin/bursaries/${b.id}/edit`)).join('')}
    </table>` : ''}
    ${urgentLearnerships.length > 0 ? `
    <div style="font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1px;margin:12px 0 6px">Learnerships</div>
    <table width="100%" cellpadding="0" cellspacing="0">
      ${urgentLearnerships.map((l) => urgentRow(l.title, l.deadline, `${ADMIN_URL}/admin/learnerships/${l.id}/edit`)).join('')}
    </table>` : ''}
    <div style="height:24px"></div>` : `
    ...
    ${newLearnerships.length > 0 ? `
    <div style="font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Learnerships</div>
    ${newLearnerships.map(rssCard).join('')}` : ''}
    ${newBursaries.length > 0 ? `
    <div style="font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1px;margin:16px 0 8px">Bursaries</div>
    ${newBursaries.map(rssCard).join('')}` : ''}
    <div style="height:24px"></div>` : ''}
    ...
    <div style="display:grid;gap:6px">
      ${[
        ...
        ['SETA Learnerships (all)', 'https://za.indeed.com/jobs?q=learnership&sort=date&fromage=7'],
        ...
        `<a href="${url}" style="display:block;font-size:12px;color:#fb923c;text-decoration:none;padding:6px 10px;background:#fff7ed;border-radius:6px">${label} ↗</a>`
      ).join('')}
    </div>

    <!-- Admin link -->
    <div style="margin-top:24px;text-align:center">
      <a href="${ADMIN_URL}/admin" style="display:inline-block;background:#fb923c;color:white;padding:12px 24px;border-radius:12px;text-decoration:none;font-weight:700;font-size:14px">
        Open Admin Panel
      </a>
    </div>

    <p style="font-size:11px;color:#cbd5e1;text-align:center;margin-top:20px">
      Sent every Monday · Vuka Career Hub
    </p>
  </div>
</div>
</body>
  ...
  const [urgentBursaries, pastBursaries, urgentLearnerships, pastLearnerships, newLearnerships, newBursaries] =
    await Promise.all([
      prisma.bursary.findMany({
        where: { isActive: true, deadline: { gte: now, lte: in45Days } },
      ...
      prisma.learnership.findMany({
        where: { status: 'OPEN', deadline: { gte: now, lte: in30Days } },
        orderBy: { deadline: 'asc' },
      }),
      prisma.learnership.findMany({
